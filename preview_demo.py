...
+    statuses = ["🟢 Active", "🟡 Processing", "🟢 Active"]
...
+        quiet_start = st.time_input("Start Time", value=datetime.strptime("22:00", "%H:%M").time(), key="quiet_start")
+        quiet_end = st.time_input("End Time", value=datetime.strptime("07:00", "%H:%M").time(), key="quiet_end")
+        
+        st.subheader("Daily Digest")
+        digest_time = st.time_input("Send Time", value=datetime.strptime("08:30", "%H:%M").time(), key="digest_time")
+        
...
+                f"+{metrics['meetings_scheduled'][1]}"
+            )
+        
+        with col4:
+            st.metric(
+                "Time Saved", 
+                f"{metrics['time_saved'][0]} hrs", 
+                f"+{metrics['time_saved'][1]} hrs"
+            )
+        
+        # Additional metrics
...
+            st.markdown(f"{activity['status']} **{activity['time']}** - {activity['action']}")
+        
...
+            agenda_icon = "📋" if meeting["agenda"] == "Generated" else "🔥"
+            
+            with st.expander(f"{agenda_icon} {meeting['time']} - {meeting['title']} ({meeting['attendees']} attendees)"):
+                if meeting['title'] == "🔥 Focus Time":
...
+                else:
+                    st.markdown(f"**Attendees:** {meeting['attendees']} people")
+                    st.markdown("**AI-Generated Agenda:**")
+                    
+                    if "Standup" in meeting['title']:
+                        st.markdown("1. Yesterday's accomplishments\n2. Today's priorities\n3. Blockers and dependencies\n4. Sprint progress update")
+                    elif "Client" in meeting['title']:
+                        st.markdown("1. Project status overview\n2. Deliverable review\n3. Client feedback session\n4. Next steps and timeline")
+                    elif "Planning" in meeting['title']:
+                        st.markdown("1. Roadmap review\n2. Feature prioritization\n3. Resource allocation\n4. Timeline planning")
+                    else:
+                        st.markdown("1. Check-in and updates\n2. Goals discussion\n3. Feedback exchange\n4. Action items")
+    
+    with tab4:
...
+            with col2:
+                st.markdown(f"{priority_color} {task['priority']}")
+            with col3:
...
+    st.markdown("*This preview shows the interface with sample data. The real agent automates your Gmail, Calendar, Notion, and messaging workflows.*")
+
